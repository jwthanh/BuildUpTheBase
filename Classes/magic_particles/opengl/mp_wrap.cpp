...
// rus: Настройка сцены
void MP_Device_WRAP::SetScene3d(bool camera_mode)
{
	#ifdef MAGIC_3D
	...
	MP_MATRIX proj;
	float wi=(float)window_width;
	float he=(float)window_height;
	if (camera_mode)
		proj.MatrixPerspective_OpenGL(27, wi/he, 0.25f, 1000000.f);
	else
	{
		proj.MatrixOrtho_OpenGL(0.f, wi, 0.f, he, 0.25f, 1000000.f);

		#ifdef MAGIC_3D
		// rus: для 3D необходимо сделать так, чтобы изображение эмиттера сместилось из левого нижнего угла в центр
		MP_MATRIX offset;
		offset.MatrixIdentity();
		offset._41=wi/2.f;
		offset._42=he/2.f;
		proj.MatrixMul(&offset, &proj);
		#endif
	}
...
void MP_Device_WRAP::SetCamera(MP_POSITION& eye, MP_POSITION& at, MP_POSITION& up)
{
