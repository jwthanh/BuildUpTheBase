...
HM_FILE MP_Platform::OpenPTC(const char* ptc_file)
{
	...
	if (LoadFile(ptc_file, &fd))
	{
		...
		HM_STREAM hmStream=Magic_StreamOpenMemory(data, (unsigned int)fd.length, MAGIC_STREAM_READ);
...
void MP_Manager::SetCamera(MP_POSITION& eye, MP_POSITION& at, MP_POSITION& up)
{
	device->SetCamera(eye, at, up);
}
#endif
...
// eng: Camera position
// rus: Положение камеры
void MP_Device::SetCamera(MP_POSITION& eye, MP_POSITION& at, MP_POSITION& up)
{
	MAGIC_CAMERA camera;
	if (camera_mode)
